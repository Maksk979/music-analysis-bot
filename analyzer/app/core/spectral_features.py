...
class SpectralFeatureExtractor:
    ...
    def extract_spectral_centroid(self, y: np.ndarray, sr: int) -> Dict[str, float]:
        c = librosa.feature.spectral_centroid(y=y, sr=sr, n_fft=self.n_fft, hop_length=self.hop_length)
        return {'spectral_centroid_mean': safe_float(np.mean(c)), 'spectral_centroid_std': safe_float(np.std(c))}

    def extract_spectral_rolloff(self, y: np.ndarray, sr: int) -> Dict[str, float]:
        r = librosa.feature.spectral_rolloff(y=y, sr=sr, n_fft=self.n_fft, hop_length=self.hop_length)
        return {'spectral_rolloff_mean': safe_float(np.mean(r)), 'spectral_rolloff_std': safe_float(np.std(r))}

    def extract_spectral_bandwidth(self, y: np.ndarray, sr: int) -> Dict[str, float]:
        b = librosa.feature.spectral_bandwidth(y=y, sr=sr, n_fft=self.n_fft, hop_length=self.hop_length)
        return {'spectral_bandwidth_mean': safe_float(np.mean(b)), 'spectral_bandwidth_std': safe_float(np.std(b))}

    def extract_chroma(self, y: np.ndarray, sr: int) -> Dict[str, Any]:
        ch = librosa.feature.chroma_stft(y=y, sr=sr, n_fft=self.n_fft, hop_length=self.hop_length)
