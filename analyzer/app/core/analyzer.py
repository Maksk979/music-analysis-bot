...
class AudioAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Анализ завершен за {features['analysis_metadata']['processing_time']:.2f} c"
            )
            return features
    ...
    def _extract_file_info(
        self, y: np.ndarray, sr: int, file_path: str
    ) -> Dict[str, Any]:
        return {
    ...
    def _compute_high_level_metrics(self, features: Dict[str, Any]) -> Dict[str, float]:
        ...
        metrics["danceability"] = float(
            np.clip(
                0.4 * metrics["energy"] + 0.4 * float(np.clip(beat_strength / 5, 0, 1)) + 0.2 * tempo_norm,
                0,
                1,
            )
        )
    ...
    def get_feature_vector(self, file_path: str) -> np.ndarray:
        ...
        vector.extend(
            [features["rhythm"]["tempo"], features["rhythm"]["mean_onset_strength"]]
        )
