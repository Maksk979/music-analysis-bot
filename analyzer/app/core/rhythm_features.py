...
class RhythmFeatureExtractor:
    ...
    def extract_tempo(self, y: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'tempo_confidence': safe_float(self._calculate_tempo_confidence(y, sr, tempo)),
            'tempo_std': tempo_std,
    ...
    def _calculate_tempo_confidence(self, y: np.ndarray, sr: int, detected_tempo: float) -> float:
        try:
            ...
            tempo_freqs = librosa.tempo_frequencies(sr=sr, hop_length=self.hop_length, n_bins=tempogram.shape[0])
            tempo_idx = np.argmin(np.abs(tempo_freqs - detected_tempo))
            confidence = safe_float(np.mean(tempogram[tempo_idx, :]) / (np.max(tempogram) + 1e-6))
